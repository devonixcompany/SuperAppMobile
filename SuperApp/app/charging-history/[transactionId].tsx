  ...
  useEffect(() => {
    const loadHistoryData = async () => {
      try {
        ...
        if (!transactionId) {
          console.warn('⚠️ [HISTORY] No transactionId provided');
          setHistoryData(null);
          setIsLoading(false);
          return;
        }

        // Fetch the specific transaction using the new endpoint
        console.log('🔍 [HISTORY] Fetching transaction:', transactionId);
        ...
        if (response.success && response.data) {
          ...
          console.log('✅ [HISTORY] Transaction data received:', JSON.stringify(transaction, null, 2));
          ...
          const mappedData: ChargingHistoryData = {
            ...
            station: {
              ...
              location: transaction.station?.location || 'Unknown Location',
              latitude: String(transaction.station?.latitude || ''),
              longitude: String(transaction.station?.longitude || ''),
              chargePointName: transaction.station?.chargePointName || 'Unknown Charge Point',
              connectorId: transaction.station?.connectorId || ''
            },
            chargePoint: {
              powerKw: transaction.chargePoint?.powerKw || 50,
              connectorType: transaction.chargePoint?.connectorType || 'Type 2',
              maxPower: transaction.chargePoint?.maxPower || 50
            },
            timing: {
              startTime: transaction.timing?.startTime || new Date().toISOString(),
              endTime: transaction.timing?.endTime || new Date().toISOString(),
              chargingDurationMinutes: transaction.timing?.chargingDurationMinutes || 0,
              completedAt: transaction.timing?.completedAt || new Date().toISOString()
            },
            charging: {
              totalEnergy: (transaction.charging?.totalEnergy || 0) / 1000, // Convert Wh to kWh
              startMeterValue: transaction.charging?.startMeterValue || 0,
              endMeterValue: transaction.charging?.endMeterValue || 0,
              currentSoC: transaction.charging?.currentSoC || 0,
              appliedRate: transaction.charging?.appliedRate || null
            },
            financial: {
              totalCost: transaction.financial?.totalCost || 0,
              totalPaid: transaction.financial?.totalPaid || 0,
              currency: transaction.financial?.currency || 'THB',
              paymentStatus: transaction.financial?.paymentStatus || 'PENDING',
              paymentMethod: transaction.financial?.paymentMethod || null,
              latestPayment: transaction.financial?.latestPayment || null
            },
            ...
            stopReason: transaction.stopReason || 'Unknown',
          ...
          console.warn('⚠️ [HISTORY] Failed to fetch transaction, response:', response);
          setHistoryData(null);
        }
      } catch (error) {
