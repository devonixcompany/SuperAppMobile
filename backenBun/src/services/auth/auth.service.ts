...
export class AuthService {
  ...
  async register(data: RegistrationData) {
    console.log('📝 [AUTH] Registration attempt:', data);
    ...
    try {
      ...
      const { firebaseUid, phoneNumber, userType, fullName, email, password } = data;
      
      // Check if user already exists by phone number
      const existingUserByPhone = await this.userService.findUserByPhoneNumber(phoneNumber);
  ...
  async login(data: LoginData) {
    ...
    try {
      ...
      const user = await this.userService.findUserByPhoneNumber("+66" + cleanedPhoneNumber);
      ...
      console.log('✅ [AUTH] User found:', { userId: user.id, status: user.status });
  ...
  async refreshToken(refreshToken: string) {
    ...
    try {
      ...
      // Check if user is active
      if (user.status !== 'ACTIVE') {
        console.log('⚠️ [AUTH] Inactive user attempted token refresh:', user.id);
