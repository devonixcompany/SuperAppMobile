 ...
 * OCPP 2.1 Adapter
 * 
 * แปลงข้อความจาก OCPP 2.1 ให้เป็น Internal Standard Message
 */

import { OCPPMessage, InternalStandardMessage } from '../types';
import { EventEmitter } from 'events';

export interface OCPP21Payload {
  [key: string]: any;
}

export class OCPP21Adapter extends EventEmitter {
  /**
   * แปลงข้อความจาก OCPP 2.1 เป็น Internal Standard Message
   */
  convertToInternal(ocppMessage: OCPPMessage, chargePointId: string): InternalStandardMessage {
    ...
    // OCPP 2.1 ใช้ messageType 2 = CALL, 3 = CALLRESULT, 4 = CALLERROR
  ...
  /**
   * แปลงข้อความจาก Internal Standard Message เป็น OCPP 2.1
   */
  ...
  /**
   * แปลง CALL message จาก OCPP 2.1
   */
  private convertCallToInternal(messageId: string, chargePointId: string, action: string, payload: OCPP21Payload): InternalStandardMessage {
    let convertedPayload = payload;
    
    // Convert specific OCPP 2.1 actions to internal format
    switch (action) {
  ...
  private getMessageTypeFromAction(action: string): number {
    const callActions = [
      ...
      'SetChargingProfile', 'ClearChargingProfile', 'SetNetworkProfile'
    ];
   ...
   */
  private convertBootNotification(payload: OCPP21Payload): any {
    return {
      chargingStation: {
   ...
   */
  private convertHeartbeat(payload: OCPP21Payload): any {
    return {
      timestamp: new Date()
    };
  }
   ...
   */
  private convertAuthorize(payload: OCPP21Payload): any {
    return {
      idToken: {
      ...
      iso15118CertificateHashData: payload.iso15118CertificateHashData
    };
  }
   ...
   */
  private convertTransactionEvent(payload: OCPP21Payload): any {
    return {
   ...
   */
  private convertMeterValues(payload: OCPP21Payload): any {
    return {
   ...
   */
  private convertStatusNotification(payload: OCPP21Payload): any {
    return {
   ...
   */
  private convertRequestStartTransaction(payload: OCPP21Payload): any {
    return {
   ...
   */
  private convertRequestStopTransaction(payload: OCPP21Payload): any {
    return {
